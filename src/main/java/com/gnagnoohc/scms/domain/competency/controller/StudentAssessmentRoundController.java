...
@RestController
@RequestMapping("/api/students/assessment-rounds")
@RequiredArgsConstructor
public class StudentAssessmentRoundController {

    private final AssessmentIntroService assessmentIntroService;
    private final StudentAssessmentRoundService studentAssessmentRoundService;

    @Operation(summary = "응시 가능한 진단 회차 목록", description = "지금이 응시기간 안이면서 로그인한 학생이 대상 조건에 "
            + "해당하는 회차를 최근 개설순으로 조회합니다. 이미 응시를 시작한 회차는 attemptId/attemptStatus가 채워집니다.")
    @GetMapping
    public ApiResponse<List<StudentAssessmentRoundResponse>> getOpenRounds(
            @AuthenticationPrincipal AuthUser authUser
    ) {
        return ApiResponse.ok(studentAssessmentRoundService.getOpenRounds(authUser.getId()));
    }

    @Operation(summary = "진단 안내 조회", description = "진단명·응시기간·문항수·예상 소요시간을 조회합니다. "
            + "이미 응시를 시작한 적이 있으면 기존 attemptId/attemptStatus를 함께 내려줍니다.")
    @GetMapping("/{roundId}/intro")
