 ...
 * 핵심역량 목록을 "읽기만" 하는 진입점.
 *
 * <p>CompetencyService(등록·축순서·사용여부 변경)와 분리한 이유:
 * <ul>
 *   <li>성격이 다르다. 이쪽은 조회 전용이라 클래스 전체를 readOnly 트랜잭션으로 묶는다.</li>
 *   <li>비교과 프로그램·마일리지 등 다른 도메인이 "핵심역량 후보 목록"을 필요로 할 때, 각자
 *       CompetencyRepository를 직접 주입하거나 Competency 엔티티를 다루지 않고 이 서비스만 호출하게 한다.
 *       후보를 고르는 규칙(활성·최상위·정렬)이 여기 한 곳에 모여 있어, 하위역량 도입 등으로 규칙이
 *       바뀌어도 이 클래스만 고치면 된다.</li>
 * </ul>
 */
@Service
...
public class CompetencyQueryService {
     ...
     * <ul>
     *   <li>includeCompetencyId가 null이면 기본 목록과 동일하다.</li>
     *   <li>이미 활성 목록에 있으면 중복해서 넣지 않는다.</li>
     *   <li>존재하지 않는 id면(참조가 꼬인 경우) 무시하고 활성 목록만 반환한다 — 수정 폼이 깨지지 않도록.</li>
     *   <li>예외로 끼워 넣는 항목은 축순서 정렬된 활성 목록 "뒤"에 붙는다(active 값으로 프론트가 구분 표시).</li>
     * </ul>
     */
