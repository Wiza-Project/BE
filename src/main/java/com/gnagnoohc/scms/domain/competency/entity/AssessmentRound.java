...
public class AssessmentRound extends BaseTimeEntity {
    ...
    // AssessmentSubmissionService.submit()이 잠금 획득 후 재검증할 때,
    // AssessmentPercentileBatchService가 잠금 획득 후 이미 처리된 회차인지 방어적으로 확인할 때 사용한다.
    public boolean isPercentileCalculationCompleted() {
        return "COMPLETED".equals(this.roundStatus);
    }
}
