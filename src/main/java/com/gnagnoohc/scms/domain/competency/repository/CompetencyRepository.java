...
public interface CompetencyRepository extends JpaRepository<Competency, Integer> {
    ...
    /**
     * 여러 화면·도메인이 "핵심역량을 고르는" 드롭다운에서 공통으로 쓰는 목록.
     * 비교과 프로그램 필터(학생), 비교과 프로그램 등록·마일리지 활동 유형 등록(교직원)에서 후보로 보여준다.
     * <ul>
     *   <li>ParentCompetencyIsNull : 하위역량은 개발 범위 밖이라(문항·정책 모두 최상위 역량에만 매핑) 최상위만 후보다.</li>
     *   <li>ActiveTrue : 비활성 역량을 새로 연결하면 이후 집계가 어긋나므로 후보에서 제외한다.</li>
     *   <li>OrderByDisplayOrderAsc : 화면마다 순서가 달라지지 않도록 축순서(결과 차트와 같은 기준)로 고정한다.</li>
     * </ul>
     */
