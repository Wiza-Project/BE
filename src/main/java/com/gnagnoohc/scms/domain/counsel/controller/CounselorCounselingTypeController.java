 ...
 *
 * <p>인가는 {@code SecurityConfig}의 URL 규칙(<code>/api/counselors/** → hasAnyRole("ST200", "ST300")</code>)이
 * 담당한다. 학생·비상담사 토큰은 이 경로에 닿기 전에 403으로 걸러지므로 메서드 보안을 별도로 걸지 않는다.
 * ST200과 ST300 겸임처럼 URL만으로 걸러지지 않는 조합은 서비스 계층의
 * {@code CounselManagementAccessPolicy}가 403(A004)으로 최종 차단한다.</p>
 */
@RestController
...
public class CounselorCounselingTypeController {
     ...
     * 목록 자체가 이미 로그인한 상담사의 역할 범위(ST200 단독/ST300 단독)로 제한되므로,
