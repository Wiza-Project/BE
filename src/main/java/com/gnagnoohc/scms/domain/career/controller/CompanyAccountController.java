...
public class CompanyAccountController {
    ...
    @Operation(summary = "협약기업 신규 등록", description = "채용공고 등록에 필요한 기업 기본 정보와 사업자등록번호를 등록합니다.")
    @PostMapping
    ...
    public ResponseEntity<ApiResponse<CompanyDetailResponseDTO>> getCompanyDetail(
            @Parameter(description = "협약기업 식별자 (PK)", example = "1")
    ...
    @Operation(summary = "기업 목록 검색 및 페이징 조회", description = "승인 상태, 기업명 등의 필터 조건으로 기업 목록을 페이징 조회합니다.")
    @GetMapping
    ...
    @Operation(summary = "기업 승인/반려 심사 처리", description = "취창업지원과 교직원이 기업의 등록 승인(VERIFIED) 또는 반려(REJECTED)를 처리합니다.")
